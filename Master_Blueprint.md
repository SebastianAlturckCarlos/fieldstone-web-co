...
| `SENDER_NAME` | — | Default `Sebastian` — inbox display name; a bare address reads as automated |
...
**Sprint 5 — Synapse Immersion. ✅ SHIPPED (2026-07-18, v1).** Click the sphere → fullscreen takeover, camera flies through the membrane (1.4s dolly) into the org rendered spatially: **CEO node at the top**, five workers arced below, live pulse traffic on the actual pipeline edges (RES→CMO→CEO→SLS→ANL + supervision spokes), node glow tracking real roster status. Click a worker → its detail panel; click the CEO → the **CEO Command console** (see Sprint 7); Escape backs out. Honors the sphere's quality tier (no post passes below tier 2). HTML labels are projected per-frame with direct DOM writes — zero per-frame React state.

**Sprint 6 — JARVIS. ✅ SHIPPED (2026-07-18, v1).** Talk to the engine. `POST /api/jarvis` + a chat panel (header toggle): status questions and commands in natural language, voice in/out via the Web Speech API (mic + speak toggles, degrades to text where unsupported). The model only acts through a **whitelisted action protocol** — approve/reject draft, send/dismiss follow-up, pause/resume sends, run digest, run Dev Agent — parsed from strict JSON; it can do nothing a dashboard button can't. Runs on Haiku via claude-code (subscription, hard-stop); a keyword fallback answers in dry-run so the interface works offline. **Verified live:** "status report" → a real Haiku-written summary of actual engine state.

**Sprint 7 — Full autonomy, perf governor, claude-code live. ✅ SHIPPED (2026-07-18).** Operator-requested sprint (post-Sprint-4 review). (1) **Performance:** the Synapse Core gained a 3-tier adaptive quality governor — measures real frame times, steps down (full cinema → no post passes/DPR 1 → static frame) after 2 consecutive <24fps seconds, detects software WebGL (SwiftShader etc) and jumps straight to static, persists the tier, pauses when the tab is hidden, and shows a tiny q-tier/fps readout. Tier <2 also sets `.perf-lean` on `<html>`, which kills the real killer: `backdrop-filter` panels re-blurring *animated* backgrounds every frame (all bg animation freezes, panel blur drops to opaque gradients). Baseline blurs reduced across the board. (2) **True automation:** the manual "Run tick" button is GONE — the engine claims, audits, drafts, QA-loops, consults, and auto-sends entirely on its own; the human's only surface is the **CEO Command console** (crown button or CEO roster/immersion click): approvals queue with the full agent chain visible, follow-up queue, pause/resume sends, digest + Dev Agent triggers. (3) **claude-code mode live on this machine:** fixed the latent `shell: true` bug in the CLI shell-out (Windows concatenates args unescaped — multi-line prompts were shredded; likely why it "worked" only untested), added an Ollama-absent fallback (researcher rides Haiku on the subscription when Qwen isn't installed, probed at most 1×/min), and the Researcher now audits the **real fetched site** (`site_fetch.ts`, 10s timeout, dead-site-as-evidence). (4) **Multi-agent collaboration:** new `sales_consult` stage — after CEO approval, the Sales Rep reviews every draft for click-through (score 0-100 + concrete notes + sharper subject); a score <60 buys one CMO revision with the Sales Rep's notes as feedback before the human sees it. The approval card now shows the whole chain: *Researcher audited · CMO drafted · CEO scored · Sales Rep consulted — you decide.* Consult runs ledger under the SLS roster light. Deterministic mocks added for consult + JARVIS so every new path works offline in dry-run.

**Sprint 8 — Command-center UX + branded-snapshot outreach. ✅ SHIPPED (2026-07-20).** Operator-directed sprint (UI/UX refactor + system execution directives). (1) **Minimalist landing:** Mission Control is now header + a full-viewport Synapse Core and nothing else — roster, live feed, KPI strip, queues, and engine controls all moved into a first-class **CEO tab** (crown button toggles a view, not a modal); pending decisions surface as one floating pill over the sphere. Immersion interior rebuilt: full agent nameplates ("CEO Agent" 22px glowing accent; workers 15px) with role + live status lines replacing the 10px 3-letter tags. (2) **Pipeline is a clickable Kanban:** 7 stage columns, cards with QA/preview/opened/reply/fail chips; clicking a card opens the **lead drill-down drawer** — audit flaws with evidence, captured brand swatches + logo, the exact snapshot as sent, every draft with QA/consult verdicts, and the per-lead agent-run + episodic-memory timeline. New endpoints: `GET /api/leads/{id}`, `GET /api/snapshots/{leadId}.png`. (3) **Outreach pivot — this supersedes Sprint 3's "personalized demo link" strategy:** live demo links are a double-edged sword (prospects click things a mock doesn't implement), so first-touch emails now embed a **screenshot of the prospect's own operations screen rendered in their brand**. The audit fetch parses brand colors (theme-color meta, else stylesheet hex-frequency with grey/extreme filtering) + logo (`img[logo]` → og:logo → apple-touch-icon → og:image) into `leads.brand_json`; a dependency-free HTML mockup (trade-aware job boards, the audit's top flaw shown solved) renders via **puppeteer-core against the machine's installed Chrome/Edge** (no bundled Chromium; `SNAPSHOT_BROWSER_PATH` override) into `snapshots/{leadId}.png` + `outreach_emails.snapshot_path`; `deliver()` sends text+HTML alternatives with the PNG inlined via Resend CID attachment. Snapshot failure is never a gate — the email ships text-only. Follow-ups re-attach the preview (the Sprint-3 note said visual proof belongs there). CMO/CEO/consult prompts rewritten: reference the embedded preview exactly once, never a URL; CEO rubric scores the preview reference; approval cards show the exact image before the human decides. `demo_link.ts` deleted. (4) **Depth unthrottled for the dedicated workstation:** BATCH_SIZE 10, MAX_QA_LOOPS 4, site excerpt 32k chars, max_tokens ~2-3× across agents, CLI `--max-turns 6` with doubled clocks. (5) **Production posture:** workspace `.env` already ran `claude-code` + `resend`; new code synced there, live DB migrated (`brand_json`, `snapshot_path`), smoke-booted clean with auto-tick off. All human gates and dispatch guards unchanged. **Verified:** standalone snapshot render pixel-checked (brand color through nav/buttons/quote panel); isolated dry-run stack ran seed → tick → 9 drafted with snapshots → approve → mock send → `sent`; headless-Chrome captures of all five screens; backend typechecks clean (`@types/node` + `@types/better-sqlite3` added). First real send day still waits only on the lead list.

**Sprint 9 — Free-agent migration, email trust fixes, competitive intelligence. ✅ SHIPPED (2026-07-22).** Operator-directed: the per-lead loop was hitting the Claude subscription's usage limit under real volume (CEO+CMO+consult+analytics, all on Sonnet/Haiku, per lead), which the engine handled wrong — a 429 permanently failed whatever lead was mid-flight instead of pausing. Rather than just fix the symptom, the whole loop moved off the subscription. (1) **Hermes migration:** `ceo_agent`, `cmo_agent`, `sales_rep_agent`, `sales_consult`, and `analytics_agent` all moved to `provider: ollama` / `hermes3:8b` (Nous Research's Hermes 3, same footprint class as the Researcher's existing `qwen2.5-coder:latest`) — research, QC, copy, consult, and the daily digest all now run at genuine $0, no subscription usage at all in normal operation. `dev_agent` deliberately stays on `claude-code`/Sonnet: it's low-frequency (weekly + on-demand) and writes real TypeScript, where local-model quality still trails meaningfully — the one place the subscription is worth spending. `worker_router.ts`'s Ollama path was generalized to route through the same `parseAgentOutput` dispatcher as claude-code (the CEO's plain-text verdict and Analytics' markdown digest can't survive a blind `format:'json'`+`JSON.parse` the way the Researcher's structured JSON always could), and prompt-variable interpolation (`{{qa_threshold}}`, `{{calendar_link}}`) now happens before the Ollama branch runs, not after — both agents that use those variables (CEO, Sales Rep) were newly exposed to a latent bug that only mattered once they left the claude-code path. The existing Ollama-down→claude-code-Haiku fallback still stands as the safety net. **429/usage-limit fix:** `orchestrator.ts`'s tick loop now recognizes usage-limit language in any thrown error (not just the typed `BudgetExceeded`) and treats it exactly the same way — requeue the lead, stop the tick, alert — instead of permanently marking the lead `failed`. (2) **Email trust fix, not a rewrite:** evaluated whether the CMO's pain-first opening (no self-introduction, straight into the costliest flaw) was actually working against the mandate for the best possible emails. Verdict: keep the opening exactly as-is — leading with a generic intro is the single most common cold-email mistake, and the existing banned-phrase list already forbids it. The real gap was identity, not structure: `SEND_FROM` had no display name (recipients saw a bare address in their inbox) and the body never signed off with an actual human name, undercutting the CMO prompt's own goal of reading "like one sharp tradesperson-adjacent operator." Fixed both — new `SENDER_NAME` env var flows into the Resend `from` field as `"Sebastian <sebastian@mail...>"`, and the CMO prompt now closes every body with a one-line first-name sign-off, right before the auto-appended company footer. (3) **Competitive intelligence:** new `competitor_agent` + `jobs/competitor_scan.ts` reuse the Researcher's existing site-fetch machinery against five direct competitors' public marketing pages (ServiceTitan, Housecall Pro, Jobber, FieldEdge, Service Fusion) — positioning, any visible pricing/contract signal, a real weakness, and one honest angle for Fieldstone's flat-$399/zero-setup-fee model to capitalize on. Runs weekly (Sunday, paired with the Dev Agent's existing cadence) or on demand; results land in `kv('competitor_intel')`, the sharpest angle flows into the CMO's payload as an optional `edge` (used only when it fits naturally, never forced), and the full note set flows into the Analytics digest. New Growth-screen panel + `POST /api/competitor-scan/run`. This is Research and Analytics "working together" on the outside view the operator asked for, using the same $0 Hermes agents as everything else. **Repo state:** this sprint builds directly on Dame's Sprint 8 (pulled clean, fast-forward — his dashboard/router hardening superseded an in-progress local redesign of the same screens). **Left open:** `hermes3:8b` needs `ollama pull hermes3:8b` on every machine that runs this for real (not yet pulled on this one at time of writing — verified via typecheck + dashboard build only, not a live run); first batch of Hermes-drafted copy and CEO verdicts should get one human eyeball pass before the next real send day, same as any model swap.
...
6. **Ollama** + `ollama pull qwen2.5-coder:latest` (Researcher) and `ollama pull hermes3:8b` (CEO, CMO, Sales Rep, Analytics, Competitor Agent) — as of Sprint 9 this makes the entire per-lead loop $0/token, not just research.
